...
const metrics={hc:pair('Численность'),hi:pair('Принятые'),ex:pair('Уволенные'),tu:pair('% текучки'),pay:pair('Общая сумма ФОТ',1e6),rev:pair('Выручка',1e6),cost:pair('Стоимость найма (на 1 чел)'),ot:pair('Переработки (часы)'),sal:pair('Средняя ЗП')};
const pr=table(await get(peopleId,'251230291')).slice(1).filter(r=>r[3]&&r[6]&&r[8]);
const ref=new Date();const ages=[],cats={},ten={'0–1 год':0,'1–3 года':0,'3–5 лет':0,'5–8 лет':0,'8–10 лет':0,'10+ лет':0};let male=0,female=0,tenureCount=0;
...
const sorted=o=>Object.entries(o).sort((a,b)=>b[1]-a[1]);const people={count:pr.length,male,female,tenureCount,ageCount:ages.length,avgAge:ages.reduce((s,x)=>s+x,0)/ages.length,medianAge:ages[Math.floor(ages.length/2)],ageBands:Object.entries(ab),categories:sorted(cats),tenure:Object.entries(ten)};
let preserved={};try{const old=await readFile('data.js','utf8');preserved=JSON.parse(old.replace(/^window\.HR_DATA=/,'').replace(/;\s*$/,''))}catch{}
if(preserved.people?.processes)people.processes=preserved.people.processes;
const next={updatedAt:new Date().toISOString(),metrics,people};
for(const key of ['efficiency','distribution','payrollDistribution'])if(preserved[key])next[key]=preserved[key];
await writeFile('data.js','window.HR_DATA='+JSON.stringify(next)+';\n','utf8');
